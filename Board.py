...
class Board(object):
    ...
    _move_connections = {
        ...
        4: [3, 9, 8],
        5: [6, 10, 0],
        6: [7, 5, 11, 1, 10, 2, 12, 0],
        7: [8, 6, 12, 2],
        8: [9, 7, 13, 3, 12, 4, 14, 2],
        9: [8, 14, 4],
        10: [11, 15, 5, 6, 16],
        11: [12, 10, 16, 6],
        12: [13, 11, 17, 7, 16, 8, 18, 6],
        13: [14, 12, 18, 8],
        14: [13, 19, 9, 18, 8],
        15: [16, 20, 10],
        16: [17, 15, 21, 11, 20, 12, 22, 10],
        17: [18, 16, 22, 12],
        18: [19, 17, 23, 13, 22, 14, 24, 12],
        19: [18, 24, 14],
        20: [21, 15, 16],
        21: [22, 20, 16],
        22: [23, 21, 17, 18, 16],
    ...
    class Move(nt):
        def __repr__(self):
            return "%s-%s-%s" % (
                Point.get_coord(self.f),
                Point.get_coord(self.t),
                self.mt.name,
            )
...
5 %s   %s   %s   %s   %s\n"""
    ...
    def _get_full_position(pos_string):
        ...
        for row in pos_string:
            ...
            assert len(row_pos) % 5 == 0, "Invaild row %s. row_pos: %s" % (
                row,
                "".join(row_pos),
            )
    ...
    def parse_position(self, position):
        ...
        assert self.goatsToBePlaced in range(21), (
            "Invalid goatsToBePlaced: %d" % self.goatsToBePlaced
        )
    ...
    def position(self):
        ...
        pos_string = "".join(
            [
                "/" * (n % 5 == 0 and n != 0) + p.get_state().name
    ...
    def is_movable(self, from_point, to_point):
        ...
        return (
            ...
            to_point in Board._move_connections[from_point]
            and
    ...
    def can_capture(self, from_point, to_point):
        ...
        return (
            # connection must exist
            to_point in Board._capture_connections[from_point]
            and
            # from_point must be a tiger
            self.points[from_point].get_state() == Point.State.T
            and
            # mid_point must be a goat
            self.points[mid_point].get_state() == Point.State.G
            and
            # to_point must be empty
            self.points[to_point].get_state() == Point.State.E
        )
    ...
    def _is_closed(self, position):
        ...
        all_goat_neighbours = any(
            [
                not self.points[i].state.name in {"T", "E"}
                for i in self._move_connections[position]
            ]
        )

        capture_tiger_present = any(
            [
                self.points[i].state.name == "T"
                for i in self._capture_connections[position]
            ]
        )
